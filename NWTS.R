...
# mice-rf
rf.imp <- mice(data = withNA.df, printFlag = FALSE, method = "rf", rfPackage = "ranger", m = m, num.threads = nthread)
...
# mixgb
xgb.params <- list(
  nthread = nthread, max_depth = 3, gamma = 0, eta = 0.3, min_child_weight = 1, subsample = 1, colsample_bytree = 1, colsample_bylevel = 1, colsample_bynode = 1,
  tree_method = "auto"
)
...
mixgb_boot.data <- mixgb(data = withNA.df, xgb.params = xgb.params, nrounds = 100, bootstrap = TRUE, m = m, maxit = 1)

mixgb_boot.fit <- lapply(mixgb_boot.data, function(dataset) coxph(model.form, data = dataset))
mixgb_boot.coefs <- do.call(cbind, lapply(mixgb_boot.fit, coef))
mixgb_boot.coefs <- rowMeans(mixgb_boot.coefs)
mixgb_boot.coefs

# final result
combine.coefs <- cbind(full.coefs, cp.coefs, mice.coefs, cart.coefs, rf.coefs, mixgb.coefs, mixgb_boot.coefs)
combine.coefs
...
mixgb_boot.colors <- c("gray40", "gray20", rep(reds[2], 5))
...
p5 <- plot_2num(
  imputation.list = mixgb_boot.data, var.x = "specwgt", var.y = "tumdiam",
...
jpeg(
  ...
  width = 15, height = 10, units = "in", res = 300, pointsize = 1
)
