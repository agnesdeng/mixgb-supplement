# simulation functions for mice
sim_mice <- function(full.df, runs = 5, m = 5, maxit = 1, seedset = NULL, model.form, p, folder.dir) {
  ...
  for (r in 1:runs) {
    ...
    # mice.default
    tic()
    mice.imp <- mice(data = withNA.df, printFlag = FALSE, m = m, maxit = maxit)
    mice.data <- complete(mice.imp, action = "all")
    sim.t <- toc(quiet = T)
    sim.time[r] <- sim.t$toc - sim.t$tic

    mice.fit <- lapply(mice.data, function(dataset) lm(formula = model.form, data = dataset))
    m.coefs[from:to, ] <- do.call(cbind, lapply(mice.fit, coef))
    result.list[from:to, ] <- summary(micombine(mice.fit))
