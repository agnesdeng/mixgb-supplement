# simulation functions for mice (rf)
sim_rf <- function(full.df, nthread = 16L, runs = 5, m = 5, maxit = 1, seedset = NULL, model.form, p, folder.dir) {
  ...
  for (r in 1:runs) {
    ...
    # mice.rf
    tic()
    rf.imp <- mice(data = withNA.df, printFlag = FALSE, method = "rf", rfPackage = "ranger", m = m, maxit = maxit, num.threads = nthread)
    rf.data <- complete(rf.imp, action = "all")
    sim.t <- toc(quiet = T)
    sim.time[r] <- sim.t$toc - sim.t$tic

    rf.fit <- lapply(rf.data, function(dataset) lm(formula = model.form, data = dataset))
    m.coefs[from:to, ] <- do.call(cbind, lapply(rf.fit, coef))
    result.list[from:to, ] <- summary(micombine(rf.fit))
