plot_var <- function(sim.output, show.vertical = T) {
  ...
  All.tb <- all.df %>%
    tibble::add_column(
      ...
      Methods = factor(rep(c("mice-default", "mice-cart", "mice-ranger", "mixgb", "mixgb-boot"), each = n_estimates),
        levels = c("mice-default", "mice-cart", "mice-ranger", "mixgb", "mixgb-boot")
      )
    ) %>%
    dplyr::mutate(
